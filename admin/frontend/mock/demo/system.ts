...
const roleList = (() => {
  ...
  for (let index = 0; index < 4; index++) {
    result.push({
      id: index + 1,
      orderNo: `${index + 1}`,
      roleName: ['슈퍼 관리자', '관리자', '게시글 관리자', '일반 사용자'][index],
      roleValue: '@first',
      createTime: '@datetime',
      remark: '@word(10,20)',
...
const menuList = (() => {
  ...
  for (let index = 0; index < 3; index++) {
    result.push({
      ...
      component: 'LAYOUT',
      type: '0',
      menuName: ['Dashboard', '권한 관리', '메뉴'][index],
      permission: '',
      orderNo: index + 1,
      createTime: '@datetime',
      'status|1': ['0', '0', '1'],
      children: (() => {
        ...
        for (let j = 0; j < 4; j++) {
          children.push({
            id: `${index}-${j}`,
            type: '1',
            menuName: ['메뉴 1', '메뉴 2', '메뉴 3', '메뉴 4'][j],
            icon: 'ion:document',
  ...
  },
  {
    ...
    response: ({ body }) => {
      ...
      if (account && account.indexOf('admin') !== -1) {
        return resultError('이 필드는 admin을 포함할 수 없습니다');
      } else {
