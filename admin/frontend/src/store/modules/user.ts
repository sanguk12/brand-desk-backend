...
import { doLogout, getCurrentUserInfo, loginApi } from '/@/api/sys/user';
...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getRoleList(): RoleInfo[] {
      return this.roleList.length > 0 ? this.roleList : getAuthCache<RoleInfo[]>(ROLES_KEY);
    },
  ...
  actions: {
    setToken(info: string | undefined) {
      ...
      if (this.token == '') {
        ...
        cookies.set('_csrf', this.token.split('_')[1]);
      }
    },
