<template>
  <PageWrapper title="가입 요청 상세">
    <template #extra>
      ...
      <a-button type="primary" :disabled='baseData.status == 2' @click="approveWithDownload"> 승인(다운로드 권한 부여) </a-button>
      <a-button type="primary" :disabled='baseData.status == 2' @click="approveWithoutDownload"> 승인(다운로드 권한 미부여) </a-button>
    </template>
    <Description
      title="기본 정보"
...
import { defineComponent, onMounted, ref } from 'vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      return { baseData, baseSchema, detailSchema, approveWithDownload, approveWithoutDownload,reject  };
    },
  });
</script>
