...
for run in {1..10}; do
  if docker pull registry.basicit.co.kr/branddesk/admin-backend-${BUILD_PROFILE}; then
      echo "success"
      break
  else
    ...
    if docker pull registry.basicit.co.kr/branddesk/admin-ui-${BUILD_PROFILE}; then
        echo "success"
        break
    else
        echo "Error Sleeping 2 seconds"
...
HOSTNAME=`hostname -f`
docker-compose --env-file ./.env -f docker-compose.yml -p admin up -d
